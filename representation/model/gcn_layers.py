...
#    edge_e = self.dropout(edge_e)
        ...
        # h_prime: N x out
        h_prime = h_prime.div(e_rowsum+1e-15)
        # h_prime: N x out
        assert not torch.isnan(h_prime).any()
...
class SPGAT(Module):
    ...
    #    edge = edge[1:, :]
        ...
        assert not torch.isnan(h).any()
        edge_h = torch.cat((h[edge[0, :], :], h[edge[1, :], :]), dim=1).t()
        # print('edge_h:', edge_h.shape, edge_h)
        # edge: 2*D x E
        edge_e = torch.exp(-self.leakyrelu(self.a.mm(edge_h).squeeze()))
        ...
        e_rowsum = self.special_spmm(edge, edge_e, torch.Size(
            [N, N]), torch.ones(size=(N, 1), device=dv))
        # print('e_rowsum:', e_rowsum.shape, e_rowsum)
        # e_rowsum: N x 1
        edge_e = self.dropout(edge_e)
        # edge_e: E
        h_prime = self.special_spmm(edge, edge_e, torch.Size([N, N]), h)
...
# signed adjacent matrix
    ...
    def forward(self, inputs, adj):
        ...
        output = torch.matmul(support.transpose(1, 2),
                              adj_norm.transpose(1, 2))
