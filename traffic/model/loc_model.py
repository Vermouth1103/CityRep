...
class LabelPredGcnModel(Module):
    def __init__(self, hparams, lane_feature, type_feature, length_feature, node_feature, adj, struct_assign, fnc_assign):
        ...
        self.linear = torch.nn.Linear(
            hparams.hidden_dims * 2, hparams.label_num)
...
class GraphEncoderTL(Module):
    ...
    def forward(self, node_feature, length_feature, adj):
        node_emb = self.node_emb_layer(node_feature)
        length_emb = self.length_emb_layer(length_feature)
...
#         cmt_emb, cmt_adj, _, _ = get_cmt(
#             inputs, adj.float(), cmt_weight_softmax)
#         cmt_emb = self.cmt_forward(cmt_emb, cmt_adj)
#         inputs = self.node_forward(
#             inputs, adj.float(), cmt_emb, cmt_weight_softmax)
#         return inputs
...
#         node_out = F.relu(self.plain_node_conv(
#             inputs_cat.unsqueeze(0), adj.unsqueeze(0))).squeeze()
#         return node_out
